...
def check_datarange(time_var, start_date_cftime, end_date_cftime, multiple=False):
    ...
    check = min_time <= start_date_cftime <= max_time and min_time <= end_date_cftime <= max_time
    # Check if the selected start and end dates are within the range
    if not multiple and check:
        print(f"The selected dates {start_date_cftime} and {end_date_cftime} are within the range of the model data.")
    elif multiple and check:
        pass
    else:
        raise ValueError(f"The selected dates {start_date_cftime} or {end_date_cftime} are out of range. Model data time range is from {min_time} to {max_time}.")
